...
final bmiProvider = FutureProvider<double?>((ref) async {
  ...
  final double? weight = latest?.weightKg ?? infoRemote?.weight ?? infoLocal.weight;
...
final weeklyLogsProvider = FutureProvider.family<List<DailyLogEntity>, int>((ref, index) async {
...
final weeklyCaloriesSeriesProvider = FutureProvider.family<List<double>, int>((ref, index) async {
...
final weeklyCaloriesTotalProvider = FutureProvider.family<double, int>((ref, index) async {
  final s = await ref.watch(weeklyCaloriesSeriesProvider(index).future);
  return s.fold<double>(0, (a, b) => a + b);
});

final weeklyCaloriesAvgProvider = FutureProvider.family<double, int>((ref, index) async {
...
final weeklyCarbsSeriesProvider = FutureProvider.family<List<int>, int>((ref, index) async {
  final logs = await ref.watch(weeklyLogsProvider(index).future);
...
final weeklyProteinSeriesProvider = FutureProvider.family<List<int>, int>((ref, index) async {
  final logs = await ref.watch(weeklyLogsProvider(index).future);
  final series = List<int>.filled(7, 0);
  for (final log in logs) {
    final dt = DateTime.parse(log.date);
...
final weeklyFatsSeriesProvider = FutureProvider.family<List<int>, int>((ref, index) async {
  final logs = await ref.watch(weeklyLogsProvider(index).future);
  final series = List<int>.filled(7, 0);
  for (final log in logs) {
    final dt = DateTime.parse(log.date);
...
final weeklyCarbsTotalProvider = FutureProvider.family<int, int>((ref, index) async {
  final s = await ref.watch(weeklyCarbsSeriesProvider(index).future);
  return s.fold<int>(0, (a, b) => a + b);
});

final weeklyProteinTotalProvider = FutureProvider.family<int, int>((ref, index) async {
  final s = await ref.watch(weeklyProteinSeriesProvider(index).future);
  return s.fold<int>(0, (a, b) => a + b);
});

final weeklyFatsTotalProvider = FutureProvider.family<int, int>((ref, index) async {
  final s = await ref.watch(weeklyFatsSeriesProvider(index).future);
  return s.fold<int>(0, (a, b) => a + b);
});

// Weight progress series (normalized 0..1)
final weightProgressSeriesProvider = FutureProvider.family<List<double>, int>((ref, index) async {
  final uc = ref.watch(getWeightRangeUseCaseProvider);
  final range = _progressRangeIso(index);
  final entries = await uc.execute(startIso: range.startIso, endIso: range.endIso);
...
final goalAchievedPercentProvider = FutureProvider.family<double, int>((ref, index) async {
  final uc = ref.watch(getWeightRangeUseCaseProvider);
  final addl = ref.watch(additionalInfoProvider);
  final target = addl.targetWeight;
  if (target == null) return 0.0;
  final range = _progressRangeIso(index);
  final entries = await uc.execute(startIso: range.startIso, endIso: range.endIso);
  if (entries.isEmpty) return 0.0;
  entries.sort((a, b) => a.date.compareTo(b.date));
  final startW = entries.first.weightKg;
  final lastW = entries.last.weightKg;
  final denom = (startW - target).abs();
  if (denom < 1e-6) return 100.0;
  final progress = ((startW - lastW).abs()) / denom;
